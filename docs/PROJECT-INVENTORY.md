...
- 通用表格筛选：一级业务模块、控制台订单、成员目录、二级功能和独立站列表的搜索、页签、状态及业务维度筛选均会真实更新列表；筛选条件同步到 URL Query，筛选无结果时提供统一空状态和清除入口。
- API 列表渲染：运营模块直接渲染服务端分页 `items`，枚举仅在展示阶段本地化；独立站列表兼容统一分页响应，接口返回空数组时不再回退到静态演示数据。
- 通用表单契约：原生输入、选择和文本域均声明稳定字段名；URL Query 与 API 参数使用英文 `snake_case`，中文只作为展示文案。构建流程会阻止缺少字段名的控件进入产物。
- 通用选项契约：所有下拉选项显式提交 `0/1`、英文枚举码或资源 ID，不再由浏览器把中文选项文案作为请求值；构建流程会拦截缺失值和中文静态值。
...
- `/features/warehouses`：仓库管理，使用 `WarehouseView.vue` 和 `/api/v1/warehouses` 读取真实仓库、成员负责人、SKU 数量与可用库存；支持创建、编辑、停用（保留历史）、负责人分配、临时/永久交接、临时交接恢复和交接历史。
