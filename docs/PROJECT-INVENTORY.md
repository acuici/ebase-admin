...
上述九个一级模块复用 `ModuleView.vue`，具备指标、页签、搜索、筛选、勾选、批量操作、表格、分页和侧边洞察。已接入 operations API 的产品、库存、用户、物流、内容、优惠券和营销列表使用服务端分页；分页器支持上一页、下一页、首尾页、相邻页和省略号，并把当前页同步到 URL Query。
