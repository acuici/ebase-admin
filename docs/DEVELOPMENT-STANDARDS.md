...
- URL Query、请求体和响应中的机器字段统一使用英文 `snake_case`。中文标签只属于展示层；禁止用中文列名、表单标签或状态文案直接充当参数名。筛选值应使用稳定枚举码，由展示映射负责本地化。
...
- 所有原生表单控件必须声明稳定、唯一且语义明确的 `name`。动态表单通过 `formFieldName` 统一映射；已接入 API 的字段必须使用显式字段名，哈希兜底只允许尚未接入后端的原型字段。
- 所有原生 `<option>` 必须显式声明 `value`，并与后端 Validate、数据库枚举或关联资源 ID 保持一致。中文仅作为选项标签。只有真正的布尔字段统一使用 `0/1`；多状态工作流不得为了节省少量存储而牺牲可读性和跨系统契约稳定性，可使用小整数枚举或英文状态码，但同一领域只能保留一套编码。
...
- 表格工具栏中出现的每一个搜索框、页签和下拉筛选都必须实际改变请求参数或当前结果集；禁止只改变按钮文案。筛选后的数量必须明确区分“当前显示”和“全量总数”。
- 已由服务端完成筛选的分页列表必须直接渲染响应中的 `items`，不得再用中文展示标签进行二次过滤；代码值到中文文案的转换只能发生在渲染阶段。空的 `items` 必须显示真实空状态，禁止因数组为空而回退到模拟数据。

## 9. UI 规范

- 所有可见文字的字号不得小于 `12px`。辅助说明、时间、徽标和表格元数据同样适用；不得以提高信息密度为由绕过。构建流程会执行 `npm run check:font-size`。
...
构建会同时执行字号门禁和表单契约门禁；缺少 `name` 的原生表单控件会直接导致构建失败。
