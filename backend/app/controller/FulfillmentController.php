...
class FulfillmentController extends ApiController
{
 ...
 public function addTracking(Request $request,int $packageId):Response{$this->validate($request->post(),['event_status'=>'require|max:64','description'=>'require|max:500','external_event_id'=>'max:128','occurred_at'=>'dateFormat:Y-m-d H:i:s'],['event_status.require'=>'物流状态不能为空','description.require'=>'轨迹描述不能为空']);return $this->success((new FulfillmentService())->addTrackingEvent($packageId,$request->post()),'物流轨迹已记录',201);}
}
