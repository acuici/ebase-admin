...
async function save(): Promise<void> {
  ...
  try {
    if (props.mode === "invite") {
      ...
      success(
        "成员邀请已创建",
        `${result.member.email} 的安全邀请令牌有效期为 24 小时`,
      );
...
async function resetPassword(): Promise<void> {
  try {
    ...
    info(
      "密码重置令牌已创建",
      `令牌有效期 ${Math.round(result.expires_in / 60)} 分钟；邮件通知接入后将自动发送。`,
    );
  } catch (exception) {
    showError(
      "操作失败",
...
<template>
  <section class="data-page member-editor">
    <button class="editor-back" @click="router.push('/settings/members')">
      <ArrowLeft :size="15" />返回成员目录
    </button>
    <div class="page-heading">
      ...
      <div class="heading-actions">
        ...
        ><button
          ...
          @click="save"
        >
          <component :is="mode === 'invite' ? Send : Save" :size="15" />{{
            saving ? "保存中..." : mode === "invite" ? "发送邀请" : "保存更改"
          }}
        </button>
      </div>
    </div>
    ...
    <div v-else class="editor-layout">
        ...
        <article class="surface editor-section">
          ...
          <div class="editor-fields">
            <label class="field-wide"
              ...
              ><small>可按住 Command / Ctrl 选择多个角色。</small></label
      ...
      <aside>
        <article class="surface editor-summary">
          <span class="member-avatar-large">{{ form.name?.[0] || "新" }}</span>
          <h3>{{ form.name || "新成员" }}</h3>
          <p>{{ form.email || "尚未填写邮箱" }}</p>
          <dl>
            <div>
              <dt>角色</dt>
              <dd>{{ form.roleIds.length }} 个已选</dd>
            </div>
            <div>
              <dt>账号状态</dt>
              <dd>
        ...
        <article v-if="mode === 'edit'" class="surface danger-zone">
          <h3>账号管理</h3>
          <button @click="resetPassword">
            <Mail :size="15" />生成密码重置令牌</button
          ><button class="danger" @click="disable">
            <Trash2 :size="15" />停用成员账号
          </button>
        </article>
      </aside>
    </div>
