...
const permissionGroups = [
  ...
  },
  {
    title: "独立站与内容",
    items: [
      "storefront.site.read",
      "storefront.site.update",
...
function rolePermissions(value: PermissionRole): string[] {
  return Array.isArray(value.permission_codes)
    ? value.permission_codes
    : value.permission_codes.split(",").filter(Boolean);
}
    ...
    <div class="workflow-top">
      ...
      <div class="workflow-actions">
        <button
          ...
          @click="save"
        >
          <Save :size="15" />{{ saving ? "保存中..." : "保存角色权限" }}
        </button>
      </div>
    </div>
    <div class="workflow-heading">
      <span class="eyebrow">ROLE PERMISSIONS · RBAC</span>
      <h1>{{ role.name || "角色权限" }}</h1>
      <p>{{ role.description || "配置功能权限和最终数据访问边界。" }}</p>
    </div>
    <article v-if="loading" class="surface form-card">
      <p>正在加载权限配置...</p>
    </article>
    <template v-else>
      <article class="surface form-card role-editor-card">
        <header>
          <h2>角色信息</h2>
          <p>角色权限由后端中间件最终校验，前端只负责配置和展示。</p>
        </header>
        <div class="form-grid">
          <label
            ><span>角色名称</span
            ><input v-model="role.name" name="name" /></label
          ><label
            ><span>状态</span
            ><select v-model="role.is_active" name="is_active">
      ...
      <article
        ...
        :key="group.title"
        class="surface permission-group"
      >
        <header>
          <div>
            <h2>{{ group.title }}</h2>
          ...
          <span
            >{{
              group.items.filter((code) => role.permission_codes.includes(code))
                .length
            }}
        ...
        <div class="permission-grid">
          <label
            ...
            :key="code"
            :class="{ checked: role.permission_codes.includes(code) }"
            ><input
              type="checkbox"
              name="permission_codes"
              :checked="role.permission_codes.includes(code)"
              @change="togglePermission(code)"
            /><ShieldCheck :size="16" /><span>{{ code }}</span></label
          >
        </div>
      </article>
    </template>
