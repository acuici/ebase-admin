...
const drawerTitle = computed(() =>
  mode.value === "create"
    ? `新增${config.value.title.replace("管理", "")}`
    : mode.value === "edit"
      ? "编辑记录"
      : "记录详情",
);
...
function openCreate() {
  ...
  form.value = config.value.columns.map((_, i) =>
    i === 0
...
function openRow(row: string[], targetMode: "view" | "edit") {
...
async function save() {
  if (!form.value[0]?.trim() || !form.value[1]?.trim()) {
    validationError.value = `${config.value.columns[0]}和${config.value.columns[1]}为必填项`;
    return;
  }
  try {
    ...
    const endpoint = typeMap[props.type];
    if (!endpoint) {
      notify("该功能暂不支持写入");
      return;
    }
    if (mode.value === "create")
      await apiRequest(`/secondary-operations/${endpoint}`, {
        method: "POST",
        body: rowPayload(),
      });
    else
      await apiRequest(`/secondary-operations/${endpoint}/${form.value[0]}`, {
        method: "PUT",
        body: rowPayload(),
      });
    notify(mode.value === "create" ? "新增记录已保存" : "修改已保存");
    drawer.value = false;
  } catch {
    notify("保存失败，请检查字段后重试");
  }
}
function askDelete(row: string[]) {
  deleteIndex.value = localRows.value.indexOf(row);
}
function confirmDelete() {
  if (deleteIndex.value >= 0) localRows.value.splice(deleteIndex.value, 1);
  persist();
  deleteIndex.value = -1;
  drawer.value = false;
  notify("记录已删除");
...
<template>
  <section v-if="config" class="secondary-page">
    <RouterLink to="/features" class="secondary-back"
      ><ArrowLeft :size="16" />返回功能地图</RouterLink
    >
    ...
    <section class="metric-strip module-metrics">
      <article v-for="m in config.metrics" :key="m[0]" class="metric-item">
        <span>{{ m[0] }}</span
        ><strong>{{ m[1] }}</strong
        ><small
          ><b>{{ m[2] }}</b></small
    ...
    <div class="stage-grid">
      ...
      >
        ...
        <ul>
          <li v-for="entry in stage.items" :key="entry">
            <i></i>{{ entry }}<MoreHorizontal :size="14" />
          </li>
        </ul>
      </article>
    </div>
    <article class="surface secondary-table">
      ...
      <div v-else class="table-scroll">
        <table>
          <thead>
            <tr>
              <th v-for="c in config.columns" :key="c">{{ c }}</th>
              <th class="crud-actions-heading">操作</th>
            </tr>
          </thead>
          <tbody>
            <tr
            ...
            >
              ...
              <td class="crud-actions">
                <button title="查看" @click="openRow(row, 'view')">
                  <Eye :size="14" /></button
                ><button title="编辑" @click="openRow(row, 'edit')">
                  <Pencil :size="14" /></button
                ><button class="danger" title="删除" @click="askDelete(row)">
                  <Trash2 :size="14" />
                </button>
              </td>
            </tr>
      ...
      <footer class="table-footer">
        <span>显示 {{ rows.length }} / {{ localRows.length }} 条记录</span>
        <div><button class="active">1</button></div>
      </footer>
    </article>
    ...
    <Teleport to="body"
        ...
        <aside class="crud-drawer">
          <header>
            <div>
              ...
              <h2>{{ drawerTitle }}</h2>
              <p>{{ config.group }} · {{ config.title }}</p>
            </div>
            <button @click="drawer = false"><X :size="18" /></button>
          </header>
          <div class="crud-form">
            <div v-if="validationError" class="crud-error">
              {{ validationError }}
            </div>
            <label v-for="(column, index) in config.columns" :key="column"
              ><span>{{ column }} <b v-if="index < 2">*</b></span
              ><select
                v-if="index === config.columns.length - 1 && mode !== 'view'"
                v-model="form[index]"
              ...
              ><input
                ...
                :class="{
                  readonly: mode === 'view',
                  invalid: validationError && index < 2 && !form[index],
                }"
            /></label>
          </div>
          <footer>
            <button
              ...
              @click="askDelete(localRows[editingIndex])"
            >
      ...
      <div v-if="deleteIndex >= 0" class="confirm-overlay">
        <div class="confirm-dialog">
          <div class="confirm-icon"><Trash2 :size="20" /></div>
          <h2>确认删除这条记录？</h2>
          <p>删除后将从当前原型数据中移除，此操作无法撤销。</p>
          <div>
            <button class="button secondary" @click="deleteIndex = -1">
              取消</button
            ><button class="button delete-button" @click="confirmDelete">
              确认删除
            </button>
          </div>
        </div>
      </div>
      <div v-if="toast" class="crud-toast">
        <Check :size="15" />{{ toast }}
      </div></Teleport
    >
  </section>
