...
const avatarSource = computed(
  () => avatarPreview.value || member.value?.avatar || "",
);
...
function chooseAvatar(event: Event): void {
  ...
  if (!file.type.startsWith("image/")) {
    showError("头像格式不支持", "请选择 JPG、PNG 或 WebP 图片");
    input.value = "";
    return;
  }
  if (file.size > 1024 * 1024) {
    showError("头像文件过大", "请选择不超过 1 MB 的图片");
    input.value = "";
    return;
  }
...
async function save(): Promise<void> {
  ...
  try {
    ...
    showError(
      "保存失败",
      exception instanceof ApiError ? exception.body.message : "请稍后重试",
    );
  } finally {
    saving.value = false;
  }
...
async function revokeOtherDevices(): Promise<void> {
  if (!confirm("确认撤销除当前设备外的全部会话？其他设备将需要重新登录。"))
    return;
  try {
  ...
  <section class="data-page profile-page">
    ...
    <div class="profile-layout">
      <aside class="surface profile-nav">
        <div class="profile-person">
          <span
            ><img
              ...
              :src="avatarSource"
              alt="个人头像"
            /><template v-else>{{ displayName[0] }}</template></span
          >
          <h3>{{ displayName }}</h3>
          <p>
            {{ form.job_title || "成员" }} ·
            {{ form.department || "未设置部门" }}
          </p>
        ...
        <button class="profile-logout" @click="signOut">
          <LogOut :size="16" />退出当前账号
        </button>
      </aside>

      <main class="profile-content">
        <article v-if="loading" class="surface editor-section">
          <p>正在加载成员资料...</p>
        </article>
        <template v-else-if="tab === 'profile'">
          <article class="surface editor-section">
            ...
            <div class="avatar-editor">
              ...
              <div>
                <strong>个人头像</strong>
                <p>建议使用正方形 JPG、PNG 或 WebP，文件不超过 1 MB。</p>
                <span
                  ><button
            ...
            <div class="editor-fields">
              <label
                ><span>姓名</span><input v-model="form.name" name="name"
              /></label>
              <label
                ><span>工作邮箱</span
                ><input :value="displayEmail" name="email" disabled
              /></label>
              <label
                ><span>手机号码</span
                ><input v-model="form.phone" name="phone" placeholder="选填"
              /></label>
              <label
                ><span>职位</span
                ><input
              ...
              <label
                ><span>界面语言</span
                ><select v-model="form.locale" name="locale">
        ...
        <template v-else-if="tab === 'security'">
          <section class="security-overview" aria-label="账号安全摘要">
            ...
            <div>
              ...
              <p>
                活动设备<b>{{ sessions.length }} 台</b>
              </p>
            </div>
            <div>
              <span><Activity :size="17" /></span>
              <p>
                近期安全事件<b>{{ authLogs.length }} 条</b>
              </p>
            </div>
          </section>
          <article class="surface editor-section security-panel">
            <header>
              ...
              <div class="security-header-actions">
                ...
                >
                  <RefreshCw
                    :size="15"
                    :class="{ spinning: securityLoading }"
                  />重新检查</button
                ><button
                  class="security-refresh danger-action"
                  :disabled="securityLoading || sessions.length < 2"
            ...
            <div v-else-if="sessions.length" class="device-list">
              <div v-for="session in sessions" :key="session.session_id">
                <span class="device-icon"
                  ><Smartphone
                    v-if="/mobile|iphone|android/i.test(session.device || '')"
                    :size="18" /><Laptop v-else :size="18"
                /></span>
                <div>
                  <b>{{ session.device || "未知设备" }}</b
                  ><small
                    ><MapPin :size="12" />{{ session.ip || "未知 IP"
                    }}<Clock3 :size="12" />最近活动
                    {{ session.last_seen || session.created_at }}</small
                  >
                </div>
                <button @click="revokeDevice(session.session_id)">
                  撤销会话
                </button>
              </div>
            </div>
            <div v-else class="security-empty compact-empty">
              ...
              <div>
                <h3>没有其他活动设备</h3>
                <p>新设备登录后会显示在这里，你可以随时撤销对应会话。</p>
              </div>
              <button class="button secondary" @click="loadSecurity">
          ...
          <article class="surface editor-section security-panel">
            <header>
              ...
              <div>
                <h2>登录与安全日志</h2>
                <p>追踪登录、登出、密码重置和会话撤销。</p>
              </div>
            </header>
            <div
              v-if="securityLoading"
              class="security-skeleton log-skeleton"
              aria-label="正在加载安全日志"
            >
              <i></i><i></i><i></i>
            </div>
            <div v-else-if="authLogs.length" class="auth-log-list">
              <div v-for="log in authLogs" :key="log.id">
                <span :data-event="log.event_type"
                  ><KeyRound :size="15"
                /></span>
                <div>
                  <b>{{ authEventLabel(log.event_type) }}</b
                  ><small>{{ log.ip || "未知 IP" }}</small>
                </div>
            ...
            <div v-else class="security-empty compact-empty log-empty">
              ...
              <div>
                <h3>暂无安全事件</h3>
                <p>登录、密码修改或设备撤销后，相关记录会自动出现在这里。</p>
              </div>
            </div>
          </article>
        </template>
        ...
        <template v-else>
          <article class="surface editor-section">
            ...
            <div class="security-options">
              <label
                ><div>
                  <b>订单与履约通知</b><small>订单状态、发货和异常提醒。</small>
                </div>
                <input
                  v-model="form.notification_preferences.order"
                  name="notify_order"
                  type="checkbox"
              /></label>
              <label
                ><div>
                  <b>库存风险通知</b><small>库存预警、补货和盘点提醒。</small>
                </div>
                <input
                  v-model="form.notification_preferences.inventory"
                  name="notify_inventory"
                  type="checkbox"
              /></label>
              <label
                ><div>
                  <b>安全通知</b><small>登录、会话和权限变化提醒。</small>
                </div>
                <input
                  v-model="form.notification_preferences.security"
