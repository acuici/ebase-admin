...
const activeMembers = computed(
  () => members.value.filter((member) => member.status === 1).length,
);
const disabledMembers = computed(
  () => members.value.filter((member) => member.status === 0).length,
);
...
<template>
  <section class="data-page member-page">
    <div class="page-heading">
      <div>
        <span class="eyebrow">ORGANIZATION · 组织权限</span>
        <h1>成员目录</h1>
        <p>管理内部员工账号、角色与账号状态。</p>
      </div>
      <div class="heading-actions">
        <button class="button secondary" @click="exportMembers">
          <Download :size="15" />导出成员
        </button>
        <button
          class="button primary"
          @click="router.push('/settings/members/invite')"
        >
          <Plus :size="16" />邀请成员
        </button>
      </div>
    ...
    <div class="metric-strip member-metrics">
      <div class="metric-item">
        <span>全部成员</span><strong>{{ total }}</strong
        ><small>当前已加载目录</small>
      </div>
      <div class="metric-item">
        <span>正常成员</span><strong>{{ activeMembers }}</strong
        ><small>可正常访问后台</small>
      </div>
      <div class="metric-item">
        <span>已停用</span><strong>{{ disabledMembers }}</strong
        ><small>会话已撤销</small>
      </div>
      <div class="metric-item">
        <span>超级管理员</span
        ><strong>{{ members.filter((m) => m.is_super === 1).length }}</strong
        ><small>拥有全部权限</small>
      </div>
    </div>
      ...
      <header class="member-toolbar">
        <label class="module-search"
          ><Search :size="16" /><input
            ...
            placeholder="搜索姓名或邮箱"
        /></label>
        <button class="button secondary" @click="loadMembers">
          <Filter :size="15" />刷新
        </button>
      </header>
        ...
        <table class="member-table">
          ...
          <tbody>
            <tr
              ...
              :key="member.id"
              @click="router.push('/settings/members/' + member.id)"
            >
              <td>
              ...
              <td>
                <span
                  class="member-status"
                  :data-status="displayStatus(member)"
                  >{{ displayStatus(member) }}</span
                >
              </td>
              ...
              <td>
                <button
                  class="row-action"
                  @click.stop="router.push('/settings/members/' + member.id)"
                >
    ...
    <div class="member-insights">
      <div>
        <Users :size="18" /><span
          ><b>{{ total }} 位成员</b>来自真实后端目录</span
        >
      </div>
      <div>
        <ShieldCheck :size="18" /><span
          ><b>{{ members.filter((m) => m.is_super === 1).length }} 位管理员</b
          >拥有全部系统权限</span
        >
      </div>
      <div>
        <UserCheck :size="18" /><span
          ><b>{{ activeMembers }} 个账号</b>当前可正常访问</span
        >
      </div>
      <div>
        <UserRoundX :size="18" /><span
          ><b>{{ disabledMembers }} 个账号</b>已停用</span
        >
      </div>
    </div>
  </section>
