...
const open = computed(() => activeLayer.value === "search");
const query = ref("");
const active = ref(0);
const input = ref<HTMLInputElement | null>(null);
const items = [
  {
    title: "订单 #EB202609010846",
    meta: "已付款 · 待揽收",
    to: "/orders/EB202609010846",
    type: "订单",
    icon: ShoppingBag,
  },
  {
    title: "Air X Pro 主动降噪耳机",
    meta: "SPU-286420 · 在售",
    to: "/products",
    type: "商品",
    icon: PackageSearch,
...
const results = computed(() => {
  ...
  return key
    ? items.filter((item) =>
        `${item.title}${item.meta}${item.type}`.toLowerCase().includes(key),
      )
    : items.slice(0, 4);
});
function show() {
...
function navigate(event: KeyboardEvent) {
  ...
  if (event.key === "ArrowUp") {
    event.preventDefault();
    active.value =
      (active.value - 1 + Math.max(results.value.length, 1)) %
...
<template>
  <button class="global-search" aria-label="打开全局搜索" @click="show">
    <Search :size="17" /><span>搜索订单、商品或用户...</span
    ><kbd>⌘ K</kbd></button
  ><Teleport to="body"
      ...
      ><div v-if="open" class="command-overlay" @click.self="close">
        ...
        >
          ...
          <div class="command-caption">
            <span>{{ query ? "搜索结果" : "最近访问" }}</span
            ><small>↑↓ 选择 · Enter 打开</small>
          </div>
          <div v-if="results.length" class="command-results">
            <button
          ...
          <div v-else class="command-empty">
            <Search :size="22" /><strong>没有找到匹配内容</strong>
            <p>尝试输入订单号、商品名称、用户姓名或报表名称。</p>
          </div>
        </section>
      </div></Transition
    ></Teleport
