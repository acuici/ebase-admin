...
const props = withDefaults(
  defineProps<{ accept?: string; maxSizeMb?: number; multiple?: boolean }>(),
  { accept: "image/*,.pdf,.xlsx,.xls,.csv", maxSizeMb: 20, multiple: true },
);
const emit = defineEmits<{ change: [files: UploadFile[]] }>();
...
function validate(file: globalThis.File) {
  ...
  if (!allowed) {
    error(
      "不支持该文件格式",
      `${file.name} 不是图片、PDF、Excel 或 CSV 文件。`,
    );
    return false;
  }
  if (file.size > props.maxSizeMb * 1024 * 1024) {
    error("文件未添加", `${file.name} 超过 ${props.maxSizeMb} MB 大小限制。`);
    return false;
  }
  return true;
}
function add(list: FileList | globalThis.File[]) {
  Array.from(list)
    .filter(validate)
    .forEach((raw) => {
...
function simulate(item: UploadFile) {
  const timer = window.setInterval(() => {
    item.progress = Math.min(
      100,
      item.progress + Math.ceil(Math.random() * 18),
    );
    if (item.progress >= 100) {
      item.state = "done";
      window.clearInterval(timer);
      timers.delete(item.id);
      success("文件上传完成", item.name);
      emit("change", files.value);
    }
  }, 180);
  ...
  <div class="file-uploader">
    ...
    >
      ...
      <div>
        ...
        ><small
          >支持图片、PDF、Excel 和 CSV，单个文件不超过 {{ maxSizeMb }} MB</small
        >
      </div>
    </button>
    <ul v-if="files.length" class="upload-list">
      <li v-for="item in files" :key="item.id">
        ...
        /><LoaderCircle
          ...
          :size="17"
        /><button v-else aria-label="重新上传" @click="retry(item)">
          <RotateCcw :size="16" /></button
        ><button aria-label="移除文件" @click="remove(item)">
          <Trash2 :size="16" />
        </button>
      </li>
    </ul>
